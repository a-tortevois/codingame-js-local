...
const getFilesListFromDir = (dirent) => {
    return readdirSync(path.join(__dirname, dirent), {withFileTypes: true}).filter((dirent) => dirent.isFile()).map((dirent) => dirent.name);
};
...
const main = () => {
    ...
    for (const testFile of inputTestFiles) {
        if (outputTestFiles.includes(testFile)) {
            testInputs = readFileSync(path.join(__dirname, 'inputs', testFile)).toString().split('\n').map((v) => v.trim());
            testOutputs = readFileSync(path.join(__dirname, 'outputs', testFile)).toString().split('\n').map((v) => v.trim());
            if (testInputs.length === 0 || testOutputs.length === 0) {
                continue;
            }
            console.warn(`-----------------------------------------------------------------------------------------------------`);
